...
WSDL.prototype.objectToXML = function (obj, name, namespace, xmlns, first, xmlnsAttr, parameterTypeObject, ancestorXmlns) {
  ...
  var ns = '';
  if (namespace != null && prefixNamespace && ((qualified || first) || soapHeader) && (!isNamespaceIgnored || this.ignoredNamespaces.indexOf(namespace) === -1)) {
    // prefix element
  ...
  if (Array.isArray(obj)) {
    ...
    var arrayAttr = self.processAttributes(obj),
      correctOuterNamespace = parentNamespace || ns; //using the parent namespace if given

    var strDefaultNamespace = 'tns:';
    if(name.indexOf(" replaceTns:true") > -1){
      name = name.replace(" replaceTns:true", "");
      
      if(name.indexOf('xmlns') > -1){
        var strTemp = name.replace(/(.*?) xmlns:(.*?)="(.*?)"/, '$2:').replace(/(.*?) xmlns:(.*?)/, '$2:');
        strDefaultNamespace = strTemp;
        name = name.replace(/(.*?) xmlns:(.*?)="(.*?)"/, '$1');
      }else{
        strDefaultNamespace = correctOuterNamespace || '';
      }
    }

    parts.push(['<', strDefaultNamespace, name, arrayAttr, xmlnsAttrib, , '>'].join(''));

    for (var i = 0, item; item = obj[i]; i++) {
      parts.push(self.objectToXML(item, name, correctOuterNamespace, xmlns, false, null, parameterTypeObject, ancXmlns));
    }

    parts.push(['</', strDefaultNamespace, name.split(" ")[0], '>'].join(''));
    ...
    for (name in obj) {
        ...
        if (self.definitions.schemas) {
          if (schema) {
            ...
            if (childParameterTypeObject && childParameterTypeObject.$type && (childParameterTypeObject.$type.indexOf('xsd') === -1)) {
              ...
              if ((typeof child == 'object' && Object.getOwnPropertyNames(child).length === 0))
                value = ['<', ns, name, ' i:nil="true" ', ' xmlns:i="http://www.w3.org/2001/XMLSchema-instance" ', '/>'].join('');
              else if ((typeof child === 'object' && name.indexOf("xmlns:d4p1") > -1) || namespace == "d4p1")
                value = self.objectToXML(child, name, "d4p1", xmlns, false, null, null, ancXmlns);
              else
      ...
      if (!Array.isArray(child)) {
        if (typeof child == 'object' && Object.getOwnPropertyNames(child).length == 0) continue;
